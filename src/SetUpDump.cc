...
void SetUpDump::SetUpDumpGeo(G4LogicalVolume* logicWorld)
{
  ...
  G4double outer_radius = 0.05*m;
  G4double inner_radius = 0.03*m;

	G4VisAttributes* SetUpDumpVisAtt;
  
	auto* solidDump = new G4Sphere("solidDump", inner_radius, outer_radius, 0.*deg, 360.*deg, 0.*deg, 160.*deg);
  auto* logicDump = new G4LogicalVolume(solidDump, Au, "logicDump");
  (void) new G4PVPlacement( nullptr, G4ThreeVector(0.,0.,20.*cm), logicDump, "physDump", logicWorld, false, 99, true); // Copy number of 1 for Dump
